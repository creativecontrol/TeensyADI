 ...
 * Adapted by Thadeus Reed for creativecontrol
 *
...
void MCP23017_multi::wiresend(uint8_t x) {
#if ARDUINO >= 100
...
uint8_t MCP23017_multi::wirerecv(void) {
#if ARDUINO >= 100
  return wire->read();
#else
  return wire->receive();
 ...
 */
uint8_t MCP23017_multi::bitForPin(uint8_t pin) { return pin % 8; }

/**
 * Register address, port dependent, for a given PIN
 */
uint8_t MCP23017_multi::regForPin(uint8_t pin, uint8_t portAaddr,
                                     uint8_t portBaddr) {
 ...
 */
uint8_t MCP23017_multi::readRegister(uint8_t addr) {
 ...
 */
void MCP23017_multi::writeRegister(uint8_t regAddr, uint8_t regValue) {
  // Write the register
  wire->beginTransmission(MCP23017_ADDRESS | i2caddr);
  wiresend(regAddr);
  wiresend(regValue);
 ...
 */
void MCP23017_multi::updateRegisterBit(uint8_t pin, uint8_t pValue,
                                          uint8_t portAaddr,
                                          uint8_t portBaddr) {
  uint8_t regValue;
 ...
 * @param sclPin alternate clock pin
 * @param sdaPin alternate data pin
 */
void MCP23017_multi::begin(TwoWire &I2Cpipe, uint8_t addr, uint8_t sclPin, uint8_t sdaPin) {
  if (addr > 7) {
    addr = 7;
  }
  i2caddr = addr;
 ...
 * @param addr Selected address
 */
void MCP23017_multi::begin(uint8_t addr) {
  if (addr > 7) {
    addr = 7;
  }
  i2caddr = addr;
 ...
 * address
 */
void MCP23017_multi::begin(void) { begin(0); }
 ...
 */
void MCP23017_multi::pinMode(uint8_t p, uint8_t d) {
 ...
 */
uint16_t MCP23017_multi::readGPIOAB() {
  uint16_t ba = 0;
  uint8_t a;
 ...
 */
uint8_t MCP23017_multi::readGPIO(uint8_t b) {
 ...
 */
void MCP23017_multi::writeGPIOAB(uint16_t ba) {
  wire->beginTransmission(MCP23017_ADDRESS | i2caddr);
  wiresend(MCP23017_GPIOA);
  wiresend(ba & 0xFF);
  wiresend(ba >> 8);
 ...
 */
void MCP23017_multi::digitalWrite(uint8_t pin, uint8_t d) {
  uint8_t gpio;
  uint8_t bit = bitForPin(pin);
 ...
 */
void MCP23017_multi::pullUp(uint8_t p, uint8_t d) {
 ...
 */
uint8_t MCP23017_multi::digitalRead(uint8_t pin) {
  uint8_t bit = bitForPin(pin);
 ...
 */
void MCP23017_multi::setupInterrupts(uint8_t mirroring, uint8_t openDrain,
                                        uint8_t polarity) {
 ...
 *
 */
void MCP23017_multi::setupInterruptPin(uint8_t pin, uint8_t mode) {
 ...
 */
uint8_t MCP23017_multi::getLastInterruptPin() {
  uint8_t intf;
 ...
 */
uint8_t MCP23017_multi::getLastInterruptPinValue() {
